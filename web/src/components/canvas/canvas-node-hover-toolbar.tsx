...
export function CanvasNodeHoverToolbar({
    ...
    const top = viewport.y + node.position.y * viewport.k - 28;
    ...
    const canAdjustFont = isText || node.type.startsWith("sticky-note:");
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...
        ...(canAdjustFont ? [{ id: "decreaseFont", title: t("canvas.nodeToolbar.decreaseFont"), label: t("canvas.nodeToolbar.zoomOut"), icon: <Minus className="size-4" />, onClick: () => onDecreaseFont(node) }] : []),
        ...(canAdjustFont ? [{ id: "increaseFont", title: t("canvas.nodeToolbar.increaseFont"), label: t("canvas.nodeToolbar.zoomIn"), icon: <Plus className="size-4" />, onClick: () => onIncreaseFont(node) }] : []),
    ...
    return (
        <>
            <div
                ...
                style={{ left, top, background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.toolbar.item, boxShadow: "0 8px 28px rgba(0,0,0,.24)" }}
            ...
            >
                {toolbarTools.map((tool) => (
                    <ToolbarAction key={tool.id} {...tool} theme={theme} showLabel={isImage ? showImageToolLabels : true} />
                ))}
                {hasImage ? <ToolbarAction id="more" title={t("canvas.imageTools.configure")} label={t("canvas.imageTools.more")} icon={<Ellipsis className="size-4" />} active={imageToolSettingsOpen} onClick={openImageToolSettings} theme={theme} showLabel={showImageToolLabels} /> : null}
            </div>
            {hasImage ? (
...
function ToolbarAction({ title, label, icon, onClick, showLabel, active = false, danger = false, theme }: ToolbarTool & { showLabel: boolean; theme: CanvasTheme }) {
    ...
    return (
        <Tooltip title={title} placement="top" mouseEnterDelay={0.2}>
            <button type="button" className={`group relative flex h-12 items-center whitespace-nowrap px-1.5 ${danger ? "text-red-400" : ""}`} onClick={onClick} aria-label={title}>
                <span
                    className={`flex h-9 items-center ${hasText ? "gap-2 px-2.5" : "justify-center px-2"} rounded-full transition`}
                    style={{ background: active ? theme.toolbar.activeBg : undefined, color: danger ? undefined : theme.toolbar.item }}
                    onMouseEnter={(event) => {
