...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], options?: RequestOptions) {
    ...
    try {
        const response = await axiosDirectThenProxy<ImageApiResponse>({ method: "post", url: aiApiUrl(requestConfig, "/images/edits"), data: formData, headers: aiHeaders(requestConfig), signal: options?.signal, timeout: IMAGE_REQUEST_TIMEOUT_MS });
        const images = await parseImagePayload(response.data);
        return images;
    } catch (error) {
        throw new Error(readAxiosError(error, apiText("requestFailed")));
