...
import { requestMedia } from "@/services/api/local-proxy";
...
async function webdavFetch(config: WebdavSyncConfig, path: string, init: RequestInit) {
    ...
    try {
        return await requestMedia(buildWebdavUrl(config, path), (url) => fetch(url, { ...init, headers, signal: controller.signal }));
    } catch (error) {
