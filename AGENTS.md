...
- 修改方案如果会削弱、绕开或推翻已经约定过的优化（例如画布展示只用 WebP、100+ 节点不解码原图、本地优先、覆盖生成），先说明冲突点、影响和取舍，等用户确认后再改代码；不要为了修当前小问题先改掉全局优化。
...
- 画布节点、元素列表、参考栏、资产卡片、生图历史等展示面只渲染本地 WebP 缩略图（图片节点 1600 长边、质量 100%），不要按缩放、悬停或“更清晰”去解码原图；画布常有 100+ 节点，加载原图会卡住拖动和缩放。原图只用于放大查看、下载、导出和发给模型的生成参考。不要为了修单个显示问题改掉这项全局性能约束。
- 批量生成、多图展示、助手面板等画布交互要尽量简洁，不要占用过多画布空间。
- 图片/视频参数面板只允许按模型脚本 `/* canvas-settings */` 注释替换已有选项或隐藏已有行，不得改成另一套动态表单或扁平控件；生成张数始终显示，不要隐藏。
...
针对当前工作区里还没提交的改动，按下面做，不要拿整站或整份待测清单过一遍。

1. 圈定范围：用 `git status` / `git diff` 和 `CHANGELOG.md` 的 `Unreleased` 列出这次改了什么；只测这些页面和交互。
2. 本地冒烟：启动项目（通常 `cd web && bun run dev`），点一遍相关页面和交互，看控制台里和这次改动有关的明显报错。不强制编译或构建。不要关掉用户已打开的浏览器窗口。
3. 人工验收：只对这次相关的 `pending-test` 条目记「通过 / 失败」。失败先修再测，不要提交。通过后从 `pending-test` 移走，用户确认过的功能再更新 `features.mdx`。
4. 确认无误后提交 Git：一次提交覆盖这批改动即可；不要改无关文件，不要升版本、不要打 tag（那是发版本流程）。
5. 推到自己的 Fork：当前在 `dev` 时执行 `git push origin dev`。不要推 `upstream`，不要 force-push，除非用户明确要求。
...
- 以后改方案时，若和已经优化过、双方确认过的做法冲突，必须先开口说明冲突在哪、为什么想改、改了会丢掉什么，等用户明确同意后再动手；不要直接改代码再解释。
...
- 正式画布本地端口是 `3099`，对应 `C:\KongCanvas` 的 `dev`。验证未合并分支时必须另开 git worktree，并用新端口（如 `3100`）启动；不要在正式工作区 checkout 分支去抢 `3099`。
- 正式画布本地端口是 `3099`，对应 `C:\KongCanvas` 的 `dev`。验证未合并分支时必须另开 git worktree，并用新端口（如 `3100`）启动；不要在正式工作区 checkout 分支去抢 `3099`。
