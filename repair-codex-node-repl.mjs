...
const commandMatch = text.match(/\[mcp_servers\.node_repl\][\s\S]*?command\s*=\s*(['"])([^'"]+node_repl\.exe)\1/i);
if (!commandMatch) process.exit(0);

const commandPath = commandMatch[2];
...
if (path.normalize(commandPath) === path.normalize(latest.exe)) process.exit(0);

const next = text.replace(commandMatch[0], commandMatch[0].replaceAll(commandPath, latest.exe));
if (next === text) process.exit(0);
fs.writeFileSync(configFile, next);
console.log(`Updated Codex node_repl runtime -> ${latest.name}`);
