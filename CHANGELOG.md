...
- [修复] 从网页复制图片后，画布 Ctrl+V 不再贴出两张。

- [新增] 图片节点右键可把原图复制到系统剪贴板，供其它应用粘贴。

- [修复] 从其它应用复制图片后，在 Edge、夸克等浏览器用 Ctrl+V 也能上传到画布。

- [调整] 「复制图片/视频」、Ctrl+C、Alt+拖动只复制结果图/视频，不带提示词、@ 资产和连线；「创建副本」仍复制提示词、@ 和连入关系。

- [调整] 画布缩放下限改为 10%；圆点背景从 50% 开始渐隐，到 10% 完全透明。

- [新增] 便利贴悬停工具栏可用「缩小 / 放大」调字号，与文本节点相同。

- [调整] 画布左侧栏「资产」再点一次会收起左侧面板。

- [修复] 长边不超过 1600 的图片也会生成 WebP 预览；`@` 芯片和参考栏在预览未就绪时回退原图，避免刚上传的 JPG 没有缩略图。

- [调整] 「我的画布」列表按最后更新时间倒序，最近改过的排在最前。

- [调整] 视频节点空闲封面的播放键改为圆角三角描边图标。

- [调整] 视频节点右键「复制图片」改为「复制视频」，复制行为不变。

- [调整] 视频截取首帧、尾帧、当前帧生成的图片节点，画布尺寸与源视频节点一致。

- [调整] 画布左侧栏上传按钮名称改为「上传」，与空白画布右键一致。

- [调整] 双击画布和左侧加号菜单去掉「上传资产」，上传只保留空白画布右键。

- [新增] 画布左侧栏「新建画布」下方增加「资产」入口，点击打开左侧面板并切到资产页。

- [调整] 画布「图片详情」预览铺满浏览器窗口，支持滚轮按光标缩放、拖动平移；缩回后点空白、关闭或 Esc 可退出。

- [调整] 图片和视频节点生成读秒改到右下角，避免挡住左上角下载和右上角展开键。

- [调整] 多图节点的下载、创建副本、设为主图改为只显示图标的圆角方按钮，样式与展开计数键一致。

- [调整] 画布展示用 WebP 缩略图改为最长边 1600、质量 100%，打开画布会重做旧缓存；原图仍只用于放大查看、下载、导出和生成参考。

- [修复] 画布图片节点改回始终渲染 WebP 缩略图，缩放和悬停不再解码原图；放大查看、下载、导出和生成参考仍用原图。

- [新增] 提示词 `@` 可引用已连接节点和我的素材；素材以原图 storageKey 挂到当前节点，删资产库后芯片仍在。视频首尾帧只认连线图且可对调，全能参考才可 `@` 图。

- [调整] 从参考图拉线新建图片节点也会记住上次模型、尺寸、质量和张数，与工具栏新建一致。

- [调整] 新建视频节点（含从参考图拖出）记住上次全部参数含清晰度；换模型后当前清晰度不在选项里时才落到该模型中间档，保证始终有选中项。

- [修复] 画布视频全屏预览按最长边完整显示，3:4 等竖屏视频不再被裁切铺满屏幕。

- [调整] 视频比例为「自动」时，按首帧参考图匹配最接近的视频比例（如 3:4）；没有参考图时仍用 16:9。

- [修复] 重试主图槽位会写回节点原图；重试优先用当前连线和参考顺序，换序后再试不会沿用旧参考快照。

- [修复] 生成成功的视频/音频会记下 generationMode，避免被当成上传资源而丢掉输入框。

- [调整] 上传的图片、视频、音频节点不再打开下方输入框，避免覆盖生成冲掉原文件；空节点和生成节点仍保留输入框。

## v0.19.0 - 2026-09-17

- [新增] 对齐上游 v0.19.0：ZCode 插件、生图 10 分钟超时、图片/文本上层按钮悬停才显示、拖拽同帧计算与连线视口裁剪。

- [调整] 画布顶栏设置、文档、插件、语言、主题和快捷键按钮平时显示与 Agent 相同的底色，悬停再略浅。

- [新增] 鼠标悬停在连线上时，光标所在位置出现圆形剪刀按钮，点击即可删除该连线且不必先选中。

- [新增] 框选连线后按 Delete / Backspace 可删除选中的连线；点选或框选时连线高亮为淡蓝 `#9ed0fe`。

- [修复] 生成和创建副本后，提示词里的参考图芯片保持常驻，不再变成纯文本「图片1」。

- [修复] 创建副本后再生成会真正换上新图，不再继续显示副本里那张旧结果。

- [修复] 连线生图时生成中的节点框不再变透明，与文生图一样显示实心底和描边。

- [修复] 提示词输入框右键改为文字复制菜单，不再弹出节点菜单。

- [修复] 视频节点空闲封面模式下仍可截取首帧、尾帧和当前帧。

- [调整] 画布图片详情标题右侧显示尺寸、体积和生成耗时。

- [修复] 空图片/视频节点切换尺寸时输入框保持原位，节点框在上方随比例变化。

- [修复] 空图片节点按竖图比例生成时不再往上跳。

- [修复] 画布生成图下载改为取原图，避免保存成 768 预览图。

- [优化] 画布节点、列表、参考栏和资产封面改用本地 WebP 缩略图（最长边 768），不写入节点数据、不参与导出和 WebDAV；放大查看、下载、导出和发给模型仍用原图。

- [优化] 画布平移缩放与节点拖动改为 DOM 直写，视频空闲只显示封面，降低上百个媒体节点时的卡顿。

- [调整] 画布参考栏和模型发送只使用连进来的图，生成结果不再自动进入当前节点参考列表。

- [新增] 画布节点右键可「复制图片」；空白画布右键可「上传」「撤销」「重做」「粘贴」，后三项右侧显示 CtrlZ / ShiftCtrlZ / CtrlV。

- [新增] 画布节点右键「创建副本」会复制提示词、模型参数、参考图上游连线和当前结果，并放到原节点下方；图片、视频、音频再次生成时覆盖当前节点，不改动原节点。

- [优化] 图片调用遇到 HTTP 451 时显示渠道/地区策略或模型权限提示，便于区分参考图编辑被上游拒绝与请求体过大。

- [修复] Yunfei 等 Gemini 渠道生图成功但只返回图片地址、包一层 data 或把图放在后续 part 时，画布不再提示未返回图片。

- [新增] 画布图片和视频节点显示生成计时：生成中实时显示秒数，完成后固定显示，点击节点后隐藏。

- [优化] 图片模型调用前按当前顺序生成参考图发送副本（最长边 1536px、无透明通道转 JPEG 0.88），并为生成结果增加 8 秒直连、90 秒本地代理下载及 413、安全拦截和下载失败提示。

- [新增] 画布参考内容支持编号、点击插入和拖动排序，提示词引用随参考位置动态更新编号，并可单独排除组内资源。

- [调整] 左侧画布工具栏顺序改为：主页、我的画布、新建画布、上传资产、下载，分割线后为插件和选择节点。

- [调整] 画布左侧节点创建菜单改为相对屏幕上下居中显示。
- [修复] 修复顶部主题切换按钮点击无响应的问题，改为直接使用主题状态切换。
- [新增] 画布节点输入框增加可自定义的提示词模板库：预装九宫格/三视图等配方，支持分组管理，点选后把完整提示词填入当前输入框。
- [调整] 画布输入框旁的模板库和提示词库图标统一悬停高亮底框样式。
- [调整] 画布模板选择面板按模板内容自适应高度，避免条目较少时出现大段空白。
- [调整] 模板管理增加可编辑的短标题，提示词编辑框改为随文案自动增长。
- [调整] 模板管理的适用场景改为随文案自动增长，长说明可完整显示。
- [调整] 模板管理左侧列表仅显示模板名称，完整适用场景集中在右侧详情展示。
- [调整] 模板管理左侧模板条目底框高度增加约 50%，名称和图标垂直居中。
- [调整] 模板选择面板外层增加主题描边，模板条目继续保持无描边。
- [调整] 提示词详情大图关闭按钮放在大图右上角；点关闭只关大图，点空白同时关闭大图和详情，且不残留蒙层。
- [修复] 提示词详情大图预览盖住详情弹窗，关闭按钮可见且可点。
- [修复] 提示词详情仍先显示限制框预览，点击预览图后再弹出完整大图。
- [新增] 提示词详情点击预览图可查看完整大图。
- [修复] 在「我的提示词」下点大类或小类会继续筛选收藏，不再回到全部。
- [调整] 提示词「已收藏」图标改为与画布删除键相同的红色。
- [修复] 提示词收藏与取消收藏会即时同步到画布侧栏「我的提示词」和提示词库，无需刷新。
- [调整] 提示词库「加入资产」改为「收藏」，收藏只进入「我的提示词」，不再写入资产库。
- [调整] 画布侧栏「提示词库」改为「我的提示词」收藏栏；点卡片在选中图/视频/文本节点时填入输入框，未选中则新建文本节点。
- [修复] 画布输入框打开提示词库后，右侧卡片列表可以滚动并继续加载，不再只显示前 20 条。
- [调整] 提示词库去掉「图像模板」大类，原小类按主题归入海报平面、插画动漫、角色人像、界面文档等分组。
- [调整] 提示词库大类标签改为加粗。
- [修复] 提示词库点选分类或标签时，左侧筛选栏保持当前滚动位置。
- [修复] 提示词库大类标签字号实际加大到 16px。
- [调整] 提示词库中文界面将英文类型标签显示为中文，模型名和来源分类保持原文；英文界面不变。
- [调整] 提示词库左侧标签筛选隐藏中文分类对应的英文重复项（如 `3d_cute`、`ui`），保留中文类型。
- [调整] 提示词库左侧标签筛选隐藏来源仓库名（如 `moosl/awsome-gpt-image-2-prompts`），「3D/手办/潮玩」这类主题标签仍保留。
- [调整] 提示词库左侧标签按视频模板、海报平面、3D相关等大类分组，大类字号更大且默认展开，可按大类或单个标签筛选。
- [调整] 提示词库左侧标签筛选隐藏全部作者名：带 `@` 的账号，以及来源 `author` 字段对应的无 `@` 名字；卡片和搜索仍保留作者标签。
- [修复] 刷新画布后不再按默认 1200px 宽度裁剪节点，当前视口内的节点应直接可见，不必先缩放。
- [新增] 图片模型脚本可用 `/* canvas-settings */` 注释声明质量、分辨率、宽高比选项并隐藏不支持的行；张数始终显示，原有控件样式不变。未声明时仍使用通用图片面板。
- [调整] Agent 插入画布的图片节点改为与画布新建一致，短边 330px 并保持原比例。
- [修复] 按原图像素计算节点尺寸时不再把超宽/超高图压成正方形，短边仍为 330px。
- [修复] 资产封面缩略图生成失败时回退原图，避免列表一直空白。
- [修复] Windows 启动脚本修复 Codex node_repl 路径时只改对应 command，避免误替换配置中的同名哈希。
- [修复] Windows 启动脚本会在 Codex 升级后自动把失效的 node_repl 路径对齐到当前运行时。
- [调整] Windows 启动脚本会同时拉起本机 Canvas Agent（127.0.0.1:17371）。
- [调整] 新建生成配置节点在画布 100% 时短边改为 330px，按原 340×240 比例约为 468×330，内部控件等比放大；已保存的 340×240 节点保持原样。
- [优化] 资产列表封面改为最长边 360px 的缩略图，避免 4K 原图撑满侧栏导致画布拖动卡顿。
- [调整] 重置视图改为将所有节点的包围盒中心对准屏幕中心，缩放仍为 100%；画布无节点时仍回到原点。
- [调整] 打组后组节点填充约为框选填充的 45%，描边改为实线；框选虚线填充不变。
- [调整] 空图片、视频、音频节点类型图标为 59px，音频改用单音符；空文本恢复编辑提示，字号 20px，文案为「双击开始编辑...」。
- [调整] 文本节点默认改为 330px × 330px；视频、音频空节点默认框为 16:9 且短边 330px，视频参数比例仍为自动；视频比例切换与上传预览与图片同一套短边计算。
- [调整] 画布 100% 时，空图片节点 1:1 为 330px × 330px，其它比例与上传预览均以 330px 为短边按比例计算。
- [修复] 文本生成失败时节点显示真实错误原因；接口返回 HTML 错误页时优先提示 HTTP 状态（如 404）。
- [新增] 视频模型脚本可用 `/* canvas-settings */` 注释声明清晰度选项和时长范围；未声明时仍使用默认 480p / 720p / 1080p 面板。
- [调整] 多图/多文本展开改为每行两张，主图留在最后一行，避免挡住下方输入框。
- [调整] 图片节点覆盖按钮改为仅图标、80% 透明底框，张数加宽 50%；收起时不显示下载角标，展开后每张图才显示下载。
- [调整] 画布节点框与描边圆角在 100% 缩放下统一为 20px。
- [调整] 拉取模型、生成、远程媒体下载和 WebDAV 均先浏览器直连，仅跨域或网络失败时回退本机代理；不再按域名写死转发。
- [调整] Windows 启动脚本会同时拉起本机 canvas-proxy。
- [修复] 图片、视频生成完成后不再按媒体原始尺寸缩小预览节点，生成中与生成后统一沿用当前比例对应的新建节点尺寸。
- [调整] 左键节点创建菜单宽度调整为 `228px`，3D 全景节点小标题简化为“360°全景查看器”。
- [调整] 图片节点参数中的生成张数改为单行 `1 / 2 / 4 / 自定义`，移除数量选项中的“张”字样。
- [调整] 节点下方输入框高度调整为 `258px`，宽度保持 `638px`。
- [调整] 左侧工具栏将扩展节点入口和分割线移动到顶部加号下方。
- [调整] 左侧工具栏移除直接创建节点入口，增加我的画布、新建画布、导入资产、导出当前画布和主页入口，并统一使用全局 Tooltip。
- [新增] 新建文本、图片、视频和音频节点时，会按节点类型沿用上一次选择的模型与生成参数。
- [修复] 兼容新版 Codex app-server 的分页线程读取，避免 `thread/read(includeTurns=true)` 错误导致本地 Agent 对话初始化失败。
- [调整] 生成过程中的停止按钮移除文字，仅保留居中的停止图标。
- [调整] 画布缩放最低限制调整为 `25%`，同步约束滚轮、滑块、定位和已保存视口。
- [调整] 新建画布的默认网格样式改为“点”。
- [调整] 新建图片节点默认尺寸改为 `236px × 236px`，视频节点保持 `420px × 236px`；两类节点的参数比例默认显示“自动”。
- [调整] 图片、视频节点切换比例后统一以 `236px` 作为短边，长边按所选比例计算。
- [调整] 新建文本节点高度调整为 `236px`。
- [调整] 画布节点与多选操作工具栏改为跟随主题的深色胶囊样式，并统一节点、生成面板和添加节点菜单的 16px 圆角。
- [调整] 将画布底部主操作菜单移动到左侧并改为纵向胶囊布局，减少对画布底部内容的遮挡。
- [优化] 左侧画布工具栏取消默认选中底框并隐藏滚动条，仅在鼠标悬停时显示按钮背景。
- [调整] 将画布工具、撤销重做、外观、删除和清空六个操作移至底部缩放栏右侧，并复用缩放栏胶囊样式。
- [调整] 深灰主题画布底色恢复为 `#0a0a0a`。
- [调整] 拉大左侧节点创建工具图标的垂直间距，使工具栏节奏更接近 Tapnow 风格。
- [调整] 左侧工具栏顶部增加亮底圆形加号占位，底部增加圆形功能占位，暂不接入具体功能。
- [调整] 将节点下方的输入框修正为居中的固定 `638px × 230px`，并抵消画布缩放对其尺寸的影响；节点框本身不做尺寸修改。
- [调整] 双击画布打开的节点创建菜单固定为 `238px` 宽，并抵消画布缩放对菜单尺寸的影响。
- [调整] 小地图向下移动并与底部缩放操作栏左对齐，保持 `8px` 间距。
- [调整] 节点悬浮操作菜单下移，与节点上边框保持 `28px` 间距，避免遮挡节点标题。
- [调整] 深灰主题点阵改为 `#464646`，并按画布缩放同步调整点阵间距和点尺寸。
- [新增] 鼠标移入左侧工具栏加号时复用节点创建菜单，并在加号右侧顶部对齐展开。
- [调整] 将上传操作从左侧工具栏移入节点创建菜单最底部。
- [调整] 将节点输入框放大按钮从底部操作栏移至输入框右上角。
- [调整] 将节点输入框底部提示词库按钮移动到右侧生成按钮旁。
- [调整] 节点输入框生成按钮改为 `36px × 36px`、圆角 `10px` 的方形，深灰主题使用 `#939393`。
- [调整] 节点连接点拖出的“引用该节点生成”菜单改为固定屏幕尺寸，不随画布缩放变化。
- [调整] “引用该节点生成”菜单整体缩小 `10%`，并保持屏幕尺寸固定。
- [调整] 双击画布节点创建菜单整体缩小 `10%`，同时保持不随画布缩放。
- [调整] 空图片、视频和音频节点统一使用等尺寸居中图标并移除空状态文字。
- [调整] 深灰主题按 Tapnow 真实画布层级更新为纯黑画布、冷灰节点、菜单及 Ant Design 浮层配色。
- [调整] 配置面板及相关编辑区域的描边改为跟随当前主题 token，提升深灰主题下的边界区分度。
- [修复] 画布节点图片/视频铺满圆角框，统一预览与高亮框圆角，确保连接点位于描边上层，并去掉底部透明溢出。
- [调整] 节点选中描边改为中性灰 `#7a7a7a`，浅色、深色、深灰主题统一。
- [新增] 画布外观新增深灰主题，顶栏明暗开关在浅色与当前暗色之间切换。
